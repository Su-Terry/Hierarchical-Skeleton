...
void Bone::rotate(GLfloat theta, Shader* shader)
{
    const vec3& axis = this->model->getPosition() - this->model->getJoint();
    this->model->rotate(axis, theta);
    for (std::forward_list<Bone*>::iterator it = this->children.begin(); it != this->children.end(); ++it)
        if (*it) (*it)->model->rotate(axis, theta);
}
